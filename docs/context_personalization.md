...
When trimming occurs, we set `state.inject_session_memories_next_turn` to trigger reinjection of session-scoped memories into the system prompt on the next turn. This preserves important short-term context across long-running sessions, while keeping the active conversation history small and within budget.
...
Below is a corrected, **fully Responses-API-consistent** version of the same pattern (state + session memory staging + trimming + tool-based distillation + end-of-session consolidation). It fixes the issues from my prior answer:

- Uses **Responses input message format** consistently (`content: [{type:"input_text", text:"..."}]`)
- Uses **Responses tool schema** correctly (`{"type":"function","function": {...}}`)
- Uses only valid **tool output items** (`{"type":"function_call_output", "call_id": ..., "output": ...}`)
- Implements **"inject session memories on the next turn after trimming"** correctly

> Note: This example keeps conversation history as chat messages only (user/assistant). That's enough for the "memory-as-state" pattern shown in [[Context Personalization with OpenAI Agents SDK]] because tool outputs are metadata-only. If you add other tools where tool results matter to the next turn, you'd also persist those results in your own state.
...
class TravelState:
    ...
    # one-shot flag: after we trim, inject session memories on the *next* model call
    inject_session_memories_next_turn: bool = False
...
# ---------------------------
# Deterministic rendering (injection)
# ---------------------------

BASE_INSTRUCTIONS = (
    "You are a concise, reliable travel concierge.\n"
    "- Ask only one focused clarifying question at a time.\n"
    "- Never invent prices/availability/policies; be explicit about uncertainty.\n"
)
...
2) SESSION overrides GLOBAL when conflicting.
3) Within a list, newest last_update_date wins.

Use memory only when relevant; don't quote it verbatim unless needed.
Never store or echo sensitive PII (passport numbers, payment details, full DOB).
</memory_policy>
""".strip()

def render_frontmatter_yaml(profile: dict) -> str:
    payload = {"profile": profile}
    y = yaml.safe_dump(payload, sort_keys=False).strip()
    return f"---\n{y}\n---"

def render_global_memories_md(notes: List[dict], k: int = 6) -> str:
    if not notes:
        return "- (none)"
    notes_sorted = sorted(notes, key=lambda n: n.get("last_update_date", ""), reverse=True)
    return "\n".join([f"- {n['text']}" for n in notes_sorted[:k]])

def render_session_memories_md(notes: List[dict], k: int = 8) -> str:
    if not notes:
        return "- (none)"
    return "\n".join([f"- {n['text']}" for n in notes[-k:]])

def build_system_prompt(state: TravelState, *, inject_session: bool) -> str:
    frontmatter = render_frontmatter_yaml(state.profile)
    global_md = render_global_memories_md(state.global_memory.get("notes", []))

    session_block = ""
    if inject_session:
        session_md = render_session_memories_md(state.session_memory.get("notes", []))
        session_block = (
            "\n\nSESSION memory (temporary; overrides GLOBAL when conflicting):\n"
            + session_md
        )
...
# ---------------------------
# Trimming: keep last N user turns (messages)
# ---------------------------

def trim_to_last_user_turns(messages: List[dict], max_turns: int) -> List[dict]:
...
INSTRUCTION_LIKE = re.compile(r"\b(always|never|system|developer|policy|instruction)\b", re.I)

def looks_sensitive_or_instruction_like(text: str) -> bool:
    if any(re.search(p, text) for p in PII_PATTERNS):
        return True
    # lightweight guardrail; tune for your domain
    if INSTRUCTION_LIKE.search(text):
        return True
    return False

def save_memory_note(state: TravelState, text: str, keywords: List[str]) -> dict:
    text = (text or "").strip()
    if not text or looks_sensitive_or_instruction_like(text):
        return {"ok": False}
    ...
    state.session_memory.setdefault("notes", []).append({
        "text": text,
...
TOOLS = [
    {
        ...
        "function": {
            ...
            "description": (
                "Save HIGH-SIGNAL, reusable travel preferences/constraints into SESSION memory. "
                "Only store durable/actionable/explicit info; no PII; no instructions."
            ),
            "parameters": {
                "type": "object",
                "properties": {
...
def dispatch_tool(state: TravelState, name: str, arguments: dict) -> dict:
...
# ---------------------------
# One turn runner (tool loop)
# ---------------------------

def run_turn(
    *,
    state: TravelState,
    history: List[dict],      # chat messages only: [{"role": "...", "content":[...]}]
    user_text: str,
    model: str = "gpt-5-mini",
    max_user_turns: int = 8,
) -> Tuple[str, List[dict]]:
    # 1) Append user, then trim
    history = history + [msg("user", user_text)]
    trimmed = trim_to_last_user_turns(history, max_turns=max_user_turns)

    trimmed_this_turn = len(trimmed) < len(history)
    history = trimmed

    # 2) Inject SESSION memories only if a previous trim requested it
    inject_session_now = state.inject_session_memories_next_turn and not trimmed_this_turn
    system_prompt = build_system_prompt(state, inject_session=inject_session_now)

    # Schedule next-turn injection if we trimmed now
    if trimmed_this_turn:
        state.inject_session_memories_next_turn = True
    elif inject_session_now:
        # one-shot: we injected them this turn, clear the flag
        state.inject_session_memories_next_turn = False

    # 3) Build Responses input (messages) and run tool loop
    input_items: List[dict] = [msg("system", system_prompt)] + history
    ...
    while True:
        ...
        function_calls = [item for item in (resp.output or []) if getattr(item, "type", None) == "function_call"]
        ...
        if function_calls:
            for call in function_calls:
                ...
                # Send tool output back as a *typed input item*
                input_items.append({
        ...
        history = history + [msg("assistant", assistant_text)]
        return assistant_text, history

# ---------------------------
# End-of-session consolidation (model-assisted)
# ---------------------------

def consolidate_memory(state: TravelState, model: str = "gpt-5-mini") -> None:
    ...
    prompt = f"""
You consolidate travel memory notes into GLOBAL long-term memory.

RULES
- Keep only durable preferences/constraints.
- Drop session-only notes (e.g., "this time", "this trip", "tonight", "today").
- Deduplicate near-duplicates.
- Conflicts: keep most recent last_update_date; ties prefer SESSION.
- Do not invent facts.
...
GLOBAL_NOTES:
{json.dumps(global_notes, ensure_ascii=False)}
...
if __name__ == "__main__":
    state = TravelState(
        profile={"home_city": "San Francisco", "tone": "concise and friendly"},
        global_memory={"notes": [
            {"text": "User usually prefers aisle seats.", "last_update_date": "2024-06-25", "keywords": ["seat"]}
        ]},
    )
    history: List[dict] = []

    out, history = run_turn(state=state, history=history, user_text="Book me a flight to Paris next month.")
    print(out)

    out, history = run_turn(state=state, history=history, user_text="Remember that I am vegetarian.")
    print(out)

    # End session
    consolidate_memory(state)
    print("GLOBAL:", state.global_memory)
```
