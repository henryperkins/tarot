...
import { Sparkle, BookOpen, CaretDown, CaretUp } from '@phosphor-icons/react';
...
export function PatternHighlightBanner({ patterns, passages, minimal = false }) {
  ...
  const totalCount = (patterns?.length || 0) + (passages?.length || 0);
      ...
      <div className="mb-4 p-3 bg-gradient-to-r from-primary/20 to-accent/20
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-xs-plus text-main/90">
            {totalCount} archetypal {totalCount === 1 ? 'insight' : 'insights'} detected
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="mb-6 p-4 bg-gradient-to-r from-primary/30 to-accent/30
      ...
      <button
        ...
        className="sm:hidden w-full flex items-center justify-between gap-2 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70 rounded"
        aria-expanded={isExpanded}
        aria-controls="pattern-highlight-content-mobile"
      >
        <div className="flex items-center gap-2">
          <Sparkle className="w-4 h-4 text-primary flex-shrink-0" />
          <span className="text-sm font-semibold text-main">Traditional Wisdom</span>
          <span className="text-xs text-muted">({totalCount})</span>
        </div>
        {isExpanded ? (
          <CaretUp className="w-5 h-5 text-primary" aria-hidden="true" />
        ) : (
          <CaretDown className="w-5 h-5 text-primary" aria-hidden="true" />
        )}
      </button>

      {/* Mobile: Collapsible content */}
      <div
        id="pattern-highlight-content-mobile"
        className={`sm:hidden ${isExpanded ? 'mt-3' : 'hidden'}`}
      >
        {/* Patterns Section */}
          ...
          <>
            ...
            <ul className="space-y-2" role="list">
              {patterns.map((pattern, i) => (
                <li key={generatePatternKey(pattern, i)} className="flex items-start gap-2 text-xs-plus leading-relaxed">
            ...
            <div className="space-y-4" role="list">
              {passages.map((passage, i) => (
                <article
                  ...
                  className="text-xs-plus text-main/90 leading-relaxed bg-primary/20 p-3 rounded border border-primary/10"
                >
                  ...
                  {passage.source && (
                    <p className="text-primary/70 text-xs-plus text-right mt-1">— {passage.source}</p>
                  )}
                </article>
              ))}
            </div>
        ...
        <p className="mt-3 pt-3 border-t border-primary/20 text-xs-plus text-muted italic">
