...
export default function StoryIllustration({ 
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (err?.name === 'AbortError') {
        setError('Illustration is taking longer than expected. Please try again in a moment.');
      } else {
  ...
  }, [cards, question, narrative, style, format, config.enabled, heroMode, onMediaReady]);
  ...
  const handleShare = useCallback(async () => {
    ...
    const fallbackMessage = 'Sharing is unavailable on this device. You can download the image instead.';
    const dataUrl = `data:image/jpeg;base64,${generatedImage}`;

    try {
      const webNavigator = typeof navigator !== 'undefined' ? navigator : null;
      if (!webNavigator?.share) {
        setStatusNotice(fallbackMessage);
        return;
      }
      ...
      const shareFile = typeof File === 'function'
        ? new File([blob], `tarot-${style}-${Date.now()}.jpg`, { type: 'image/jpeg' })
        : null;
      ...
      if (typeof webNavigator.canShare === 'function' && !webNavigator.canShare({ files: [shareFile] })) {
        setStatusNotice(fallbackMessage);
        return;
      }

      await webNavigator.share({
        title: 'Tarot Story Illustration',
        text: 'Generated from my tarot reading.',
        files: [shareFile]
      });
      setStatusNotice('Illustration shared.');
    } catch (err) {
      if (err?.name === 'AbortError') return;
      if (err?.name === 'NotAllowedError') {
        setStatusNotice('Share canceled.');
        return;
      }
      setStatusNotice('Unable to share right now. Please try again.');
    }
  }, [generatedImage, style]);
  ...
  if (!config.enabled && !generatedImage) {
    return (
      <div className={`${embedded ? 'p-4 sm:p-5 bg-surface/70' : 'p-6 bg-surface/95 backdrop-blur-xl'} rounded-xl border border-secondary/40 ${className}`}>
  ...
  return (
    <div className={containerClassName}>
      {heroBackground}
      <div className={`${embedded ? 'mb-4' : 'text-center mb-6'}`}>
        <h3 className="text-lg font-serif text-accent mb-1">
          {embedded ? 'Illustration Generator' : 'Visual Companion'}
        </h3>
        <p className="text-muted text-sm">
          Generate artwork anchored to this exact narrative and spread.
        </p>
      </div>
      ...
      {!generatedImage && !loading && (
        <div className="space-y-4 transition-opacity duration-200 ease-out">
          ...
          {/* Generate button */}
          <div className={`pt-2 ${embedded ? '' : 'text-center'}`}>
            <button
      ...
      {generatedImage && (
        <div className="transition-opacity duration-200 ease-out">
          <GeneratedArt
            ...
            canShare={Boolean(generatedImage)}
            showImage
          />
      ...
      {statusNotice ? (
        <div className="mt-3 text-center">
          <p className="text-xs text-muted">{statusNotice}</p>
        </div>
      ) : null}
