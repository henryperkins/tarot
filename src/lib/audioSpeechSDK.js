 ...
 * @param {string} [locale='en-US'] - BCP-47 locale, or empty for all voices
 * @returns {Promise<Array<Object>>} Array of voice info objects
 */
export async function getAvailableVoices(locale = 'en-US') {
  if (!isSpeechSDKReady()) {
    console.warn('[SpeechSDK] Not initialized');
    return [];
  }
  ...
  try {
    ...
    if (result.reason === sdk.ResultReason.VoicesListRetrieved) {
      return result.voices.map(v => ({
        ...
        supportsStyle: STYLE_ENABLED_VOICES.has(v.shortName)
      }));
    }

    console.warn('[SpeechSDK] Failed to get voices:', result.reason);
    return [];
  } catch (err) {
    console.warn('[SpeechSDK] Voice list error:', err);
    return [];
  } finally {
    closeSynthesizer(synthesizer);
  }
...
/**
 * Set the default voice for new synthesis requests.
 * @param {string} voice - Voice name key (e.g., 'verse') or Azure voice name
 */
export function setVoice(voice) {
  if (!speechConfig) return;
  currentVoice = VOICE_MAP[voice] || voice;
  speechConfig.speechSynthesisVoiceName = currentVoice;
}
