...
class CelticCrossBuilder extends BaseSpreadBuilder {
  constructor(options = {}) {
    super(options);
  }

  buildNucleusSection(nucleus, cardsInfo, themes, context, reasoning = null, proseOptions = {}) {
  ...
  buildTimelineSection(timeline, cardsInfo, themes, context, reasoning = null, proseOptions = {}) {
  ...
  buildConsciousnessSection(consciousness, cardsInfo, themes, context, reasoning = null, proseOptions = {}) {
  ...
  buildStaffSection(staff, cardsInfo, themes, context, reasoning = null, proseOptions = {}) {
  ...
  buildCrossChecksSection(crossChecks, themes, proseOptions = {}) {
  ...
  async buildSynthesisSection(cardsInfo, themes, celticAnalysis, userQuestion, context, remedyRotationIndex = 0, proseOptions = {}) {
  ...
  async buildNarrative({
    ...
    recordEnhancedSection(
      this.buildNucleusSection(celticAnalysis.nucleus, cardsInfo, themes, context, reasoning, proseOptions),
      {
        type: 'nucleus',
    ...
    recordEnhancedSection(
      this.buildTimelineSection(celticAnalysis.timeline, cardsInfo, themes, context, reasoning, proseOptions),
      { type: 'timeline' }
    );

    // 3. CONSCIOUSNESS - Subconscious, Center, Conscious (Cards 6-1-5)
    recordEnhancedSection(
      this.buildConsciousnessSection(celticAnalysis.consciousness, cardsInfo, themes, context, reasoning, proseOptions),
      { type: 'consciousness' }
    );

    // 4. STAFF - Self, External, Hopes/Fears, Outcome (Cards 7-10)
    recordEnhancedSection(
      this.buildStaffSection(celticAnalysis.staff, cardsInfo, themes, context, reasoning, proseOptions),
      { type: 'staff' }
    );

    // 5. CROSS-CHECKS - Key position comparisons
    recordEnhancedSection(
      this.buildCrossChecksSection(celticAnalysis.crossChecks, themes, proseOptions),
      { type: 'relationships' }
    );
    ...
    recordEnhancedSection(
      await this.buildSynthesisSection(cardsInfo, themes, celticAnalysis, userQuestion, context, remedyRotationIndex, proseOptions),
      { type: 'outcome' }
    );
...
export async function buildCelticCrossReading(payload, options = {}) {
  const builder = new CelticCrossBuilder(options);
