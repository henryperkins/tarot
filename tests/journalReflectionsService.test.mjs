...
async function postHttp(env, body, { entryId = 'entry-1', cookie = 'session=session-1' } = {}) {
...
async function post(env, input, { entryId = 'entry-1' } = {}) {
  const result = await addJournalReflection({ env, user: OWNER, entryId, input, policy: 'mcp' });
  return { response: { status: result.status }, payload: result.body };
}
...
describe('reflections: access', () => {
  it('answers 401 without credentials', async () => {
    ...
    const { response, payload } = await postHttp(env, { text: 'note', scope: 'reading' }, { cookie: null });
  ...
  it('answers a tier-limited 403 below Plus', async () => {
    ...
    const { response, payload } = await postHttp(env, { text: 'note', scope: 'reading' });
  ...
  it("answers the same 404 for a missing entry and for another user's entry", async () => {
    const { d1, env } = await setup();
    const missing = await postHttp(env, { text: 'note', scope: 'reading' }, { entryId: 'nope' });
    const foreign = await postHttp(env, { text: 'note', scope: 'reading' }, { entryId: 'entry-other' });
...
describe('reflections: append and idempotency', () => {
  ...
  it('preserves distinct Windows line endings and trailing spaces as different notes', async () => {
    const { d1, env } = await setup();
    await post(env, { text: 'line one\nline two', scope: 'reading' });
    const retry = await post(env, { text: 'line one\r\nline two  ', scope: 'reading' });
    assert.equal(retry.payload.alreadyPresent, undefined);
...
describe('addJournalReflection: compare-and-swap', () => {
  ...
  it('answers 409 when contention exhausts the bounded retry budget', async () => {
    const { d1 } = await setup();
    let attempts = 0;
